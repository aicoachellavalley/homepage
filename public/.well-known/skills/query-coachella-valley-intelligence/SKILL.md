...
AICV operates an MCP (Model Context Protocol) server. The canonical endpoint is `https://mcp.aicoachellavalley.com/mcp` (the bare host accepts POST as an alias). Use stateless Streamable HTTP: initialize with protocol `2025-06-18`, send `Accept: application/json, text/event-stream`, and include the negotiated `MCP-Protocol-Version` header on subsequent requests. The desk returns JSON, accepts notifications with an empty 202 response, and returns 405 for `GET /mcp` because it does not offer a server-initiated stream. Clients must send the Accept header under the protocol; this desk nevertheless tolerates its absence instead of returning 406. The server card is published at `/.well-known/mcp/server-card.json` and lists six tools:
...
These JSON endpoints are published as static files on the site. The collection indexes support browsing; node-detail records carry the complete body:

- `https://aicoachellavalley.com/nodes.json` — all nodes with metadata
- `https://aicoachellavalley.com/nodes/[slug].json` — one complete node body as readable Markdown, with its canonical page and content date; generated by the same build as the human page
