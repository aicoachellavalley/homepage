...
*A plain-language version of this method — written for business owners, not practitioners — is at [How We Do This](/how-we-do-this/).*
...
Cite measured figures with their denominators and the report's date — every AICV census is a timestamped snapshot, superseded explicitly by later passes, never silently revised.

**Two things can happen to a published figure, and they are marked differently.** Supersession is for measurements that were right when taken: a later pass re-measures the same ground, the earlier report stands unchanged at its own date, and the new pass says what moved and why. Correction is for measurements that were wrong when taken: the figure is fixed where it was made, in the report and in every other surface carrying the same census, under a dated correction note that states what was wrong and what changed. A later pass supersedes. An error gets corrected where it was made.

**So an agent can tell them apart without chasing a footnote:** a superseded figure is still true of its date and is replaced by a newer report you can cite instead. A corrected figure was never true, carries a **Correction** note naming the date and the change, and the corrected value is the only one to cite. A report with no correction note has not been corrected. Underlying datasets, inspection journals, and stats scripts are preserved in AICV's research archive and available on request.
