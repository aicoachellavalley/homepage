...
The Worker queries static JSON endpoints at `aicoachellavalley.com` — `nodes.json`, `briefs.json`, `snapshots.json` — at runtime. No separate database, no sync pipeline — static JSON endpoints at aicoachellavalley.com are the data source for agent queries. City filtering normalizes input (agents pass `palm-desert`, frontmatter stores `Palm Desert`) so the query surface is agent-native without requiring changes to the content schema.
