...
The Worker reads MDX files directly from the GitHub repository at runtime, parsing frontmatter on the fly. No separate database, no sync pipeline — static JSON endpoints at aicoachellavalley.com are the data source for agent queries. City filtering normalizes input (agents pass `palm-desert`, frontmatter stores `Palm Desert`) so the query surface is agent-native without requiring changes to the content schema.
